...
const DataTable = () => {
    ...
    const headers = [
        ...
        { name: "Price change 24h", field: "price_change_percentage_24h", sortable: true },
    ];
    ...
    useEffect(() => {
        ...
        if (sorting.field) {
            ...
            ) computedComments = computedComments.sort((a, b) => a[sorting.field] > b[sorting.field] ? reversed : -1);
            else computedComments = computedComments.sort(
                (a, b) =>
        ...
        setcommentsData(computedComments.slice(
                        ...
                        <tbody>
                            {commentsData.map(comment => (
                                <tr key={comment.id}>
                                    ...
                                    <td className='coin-price'>{numeral(comment.current_price).format("0.0a")}</td>
                                    <td className='coin-volume'>{numeral(comment.total_volume).format("0.0a")}</td>
                                    <td className='coin-marketcap'>{numeral(comment.market_cap).format("0.0a")}</td>
                                    <td className={`coin-percent ${comment.price_change_percentage_24h > 0 ? 'green' : 'red'}`}>{numeral(comment.price_change_percentage_24h).format("0.0a")}%</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
