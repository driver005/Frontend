...
function Charts(
    ...
    const handleTooltip = ({ event, data, xData, xScale, yScale }) => {
        ...
        const d1 = data[index];
        if(d0 && d1) {
            const d = xScale(x0) - xScale(all(d0)) > xScale(all(d1)) - xScale(x0) ? d1 : d0;
    ...
    if (data.length > 0) {
        ...
        const setTooltipLabelTop = ((maxTooltipTop - tooltipTop) > 20) ? tooltipTop - 50 : maxTooltipTop - 75;

        return (
            <div>
                <svg className={"monospace"} style={{ overflow: 'visible' }} width={width} height={height + 30}>
                    <LinearGradient
                        id="fill"
                        from="#6086d6"
                        to="#6086d6"
                    ...
                    <Group>
                        <AxisBottom
                            ...
                            ticksComponent={({ formattedValue, ...tickProps }) => {
                                return (
                                    { ...tickProps }.ticks.map((data, index) => (
                                    <text
                                        key={index}
                        ...
                        <AreaClosed
                            ...
                            fill="url(#dLines)"
                        />
                        <LinePath curve={enableCurveStepAfter ? curveStepAfter : curveLinear} stroke={"#6086d6"} data={data} y={d => yScale(y(d))} x={d => xScale(xData(d))} />
                        <MinPrice
                            data={minValuesData}
                ...
                {tooltipData &&
                    <Tooltips
                        ystyle={{ transform: tooltipValueTranslate, transition: tooltipAnimation}}
                        yclassName={"y-tooltip"}    
                        yTop={setTooltipLabelTop}
                        yLeft={tooltipLeft + 12}
                        yLabel={`Closed ${numeral(formatPrice(y(tooltipData))).format("0.0a")} \nOpen ${numeral(formatPrice(open(tooltipData))).format("0.0a")} \nHigh ${numeral(formatPrice(high(tooltipData))).format("0.0a")} \nLow ${numeral(formatPrice(low(tooltipData))).format("0.0a")} \nVolume ${numeral(formatPrice(volume(tooltipData))).format("0.0a")}`}
                        xstyle={{ transform: tooltipDateTranslate, transition: tooltipAnimation }}
